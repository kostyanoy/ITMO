...
public class Space extends Location {
    public Space(String name) {
        ...
        shell.addAction(new Action("Закрыть дверцу", p -> p.getName() + " закрыл дверцу шкафа", p -> shell.setDescription("Шкаф для книг с закрытой дверцей."), true));
        addAction(new Action("Прислушаться к ощущениям", p -> String.format("Вместе с головокружением у %s появилось какое-то странное ощущение зависания вниз головой, то есть ему на какой-то миг показалось, будто он перевернут кверху ногами", p.getName()), p -> {
